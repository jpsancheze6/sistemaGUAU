    ...
    public void create(Usuario usuario) {
        ...
        try {
            ...
            for (Factura facturaListFactura : usuario.getFacturaList()) {
                Usuario oldUsuarioidOfFacturaListFactura = facturaListFactura.getUsuarioid();
                facturaListFactura.setUsuarioid(usuario);
                facturaListFactura = em.merge(facturaListFactura);
                if (oldUsuarioidOfFacturaListFactura != null) {
                    oldUsuarioidOfFacturaListFactura.getFacturaList().remove(facturaListFactura);
                    oldUsuarioidOfFacturaListFactura = em.merge(oldUsuarioidOfFacturaListFactura);
                }
            }
            for (ReciboCompra reciboCompraListReciboCompra : usuario.getReciboCompraList()) {
                Usuario oldUsuarioidOfReciboCompraListReciboCompra = reciboCompraListReciboCompra.getUsuarioid();
                reciboCompraListReciboCompra.setUsuarioid(usuario);
                reciboCompraListReciboCompra = em.merge(reciboCompraListReciboCompra);
                if (oldUsuarioidOfReciboCompraListReciboCompra != null) {
                    oldUsuarioidOfReciboCompraListReciboCompra.getReciboCompraList().remove(reciboCompraListReciboCompra);
                    oldUsuarioidOfReciboCompraListReciboCompra = em.merge(oldUsuarioidOfReciboCompraListReciboCompra);
                }
            }
        ...
        try {
            ...
            for (Factura facturaListOldFactura : facturaListOld) {
                if (!facturaListNew.contains(facturaListOldFactura)) {
                    ...
                    illegalOrphanMessages.add("You must retain Factura " + facturaListOldFactura + " since its usuarioid field is not nullable.");
                }
            }
            for (ReciboCompra reciboCompraListOldReciboCompra : reciboCompraListOld) {
                if (!reciboCompraListNew.contains(reciboCompraListOldReciboCompra)) {
                    ...
                    illegalOrphanMessages.add("You must retain ReciboCompra " + reciboCompraListOldReciboCompra + " since its usuarioid field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (Factura facturaListNewFactura : facturaListNew) {
                if (!facturaListOld.contains(facturaListNewFactura)) {
                    Usuario oldUsuarioidOfFacturaListNewFactura = facturaListNewFactura.getUsuarioid();
                    facturaListNewFactura.setUsuarioid(usuario);
                    facturaListNewFactura = em.merge(facturaListNewFactura);
                    if (oldUsuarioidOfFacturaListNewFactura != null && !oldUsuarioidOfFacturaListNewFactura.equals(usuario)) {
                        oldUsuarioidOfFacturaListNewFactura.getFacturaList().remove(facturaListNewFactura);
                        oldUsuarioidOfFacturaListNewFactura = em.merge(oldUsuarioidOfFacturaListNewFactura);
                    }
                }
            }
            for (ReciboCompra reciboCompraListNewReciboCompra : reciboCompraListNew) {
                if (!reciboCompraListOld.contains(reciboCompraListNewReciboCompra)) {
                    Usuario oldUsuarioidOfReciboCompraListNewReciboCompra = reciboCompraListNewReciboCompra.getUsuarioid();
                    reciboCompraListNewReciboCompra.setUsuarioid(usuario);
                    reciboCompraListNewReciboCompra = em.merge(reciboCompraListNewReciboCompra);
                    if (oldUsuarioidOfReciboCompraListNewReciboCompra != null && !oldUsuarioidOfReciboCompraListNewReciboCompra.equals(usuario)) {
                        oldUsuarioidOfReciboCompraListNewReciboCompra.getReciboCompraList().remove(reciboCompraListNewReciboCompra);
                        oldUsuarioidOfReciboCompraListNewReciboCompra = em.merge(oldUsuarioidOfReciboCompraListNewReciboCompra);
                    }
                }
            }
            em.getTransaction().commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            for (Factura facturaListOrphanCheckFactura : facturaListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Usuario (" + usuario + ") cannot be destroyed since the Factura " + facturaListOrphanCheckFactura + " in its facturaList field has a non-nullable usuarioid field.");
            }
            ...
            for (ReciboCompra reciboCompraListOrphanCheckReciboCompra : reciboCompraListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Usuario (" + usuario + ") cannot be destroyed since the ReciboCompra " + reciboCompraListOrphanCheckReciboCompra + " in its reciboCompraList field has a non-nullable usuarioid field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
